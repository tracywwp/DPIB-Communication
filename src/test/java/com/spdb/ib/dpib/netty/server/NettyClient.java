...
public class NettyClient {
	 ...
	 * @param message
	 */
	public NettyClient(String host, int port, Object message) {
		this.host = host;
		this.port = port;
		this.message = message;
	}

	public Object StartClient() {
		Channel channel = null;
		ClientBootstrap bootstrap = null;
		try {
			System.out.println("客户端开始启动并连接服务器..........");
			// 构建客户端
			...
			if (!future.isSuccess()) {
				...
				System.out.println("客户端未连接到服务器,请查看ip,端口号是否正确.......");
			...
			while (true) {
				System.out.println("客户端向服务器发送数据:" + message);
			...
			System.out.println("客户端并连接服务器发生异常,请查看ip,端口号是否正确.......");
			channel.close().awaitUninterruptibly();
			bootstrap.releaseExternalResources();
		} finally {
			// 关闭连接
